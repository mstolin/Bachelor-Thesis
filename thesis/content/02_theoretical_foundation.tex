...
% Short Abstract
A deployment pipeline is a implementation of the process for getting software from source code to production.
% Based on CI
...
% FIrst part
CI is the first part in a deployment pipeline.
% What its about
The CI process is responsible for building, testing and validating the software.
This ensures that the software is in a deployable state at all time \cite{Rossel2017CICD}.
% Requirements
A CI pipeline is based on several requirements:
% The list
\begin{enumerate}
\item \textbf{A single code base:} It is important that the source code of an application is in a single code base.
% Automation script
This includes all automation script to build, test and validate the application.
% Changes
Each change on the code base will trigger the CI pipeline on the build server \cite{Rossel2017CICD}.

\item \textbf{A build server:} The build server is responsible to monitor the code base for changes.
% Execute
If a change has been pushed, the build server automatically executes the CI scripts in order \cite{Rossel2017CICD}.
\end{enumerate}

% Goal of CI
The goal of  CI pipeline is, to have a production ready artefact of the software after performing \cite{Rossel2017CICD}.
