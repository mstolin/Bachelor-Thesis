...
\subsubsection{Continuous Integration Process Implementation Example}
% Explain workflow
...
% Feedback
Finally, the CI server sends feedback to inform the developer bout the build script status  \cite{Duvall2007CI}.
...
% Automated scripts
All assets to perform the CI run should be able to access from the repository. Therefore a machine can start the build script process by executing a command script in an automated fashion \cite{Duvall2007CI}.
% The example
