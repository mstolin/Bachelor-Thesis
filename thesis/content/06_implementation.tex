...
To estimate if a scaling action is necessary, the Scaling Heat algorithm is used (introduced in Section AB).
% For each performance metric
During each interval, the heat will be calculated for each performance metric specified in the configuration under \textit{metrics}.
% HeatStore
To store and calculate the heat for each performance metric, a class called \textit{HeatStore} was created.
% Explain UML
FIGURE XY describes the UML class diagram for the \textit{HeatStore} Python class.
% For what can it be used
The class can be used to retrieve, update and reset the heat for a performance metric.


% Recurrence factor
\Tab{table:06_auto-scaler_config_parameter} shows a configuration parameter called \textit{recurrence\_factor} under general.
% Indication
The recurrence factor (N) indicates if a scale-out or scale-in action is necessary. If the calculated heat for a performance metric is N then a scale-out action will be performed, if the heat is -N, a scale-in action will be performed.
% Reset heat
After a scaling action has been performed, the heat of the performance metric will be reset.
...
For this project, scaling actions will be performed according to the CPU and GPU utilization. The number of needed worker nodes will be calculated for each performance metric. At then end, the highest number of worker nodes will be chosen KUBERNETES QUELLE.
...
% Cooldown
After a scaling action has been performed, a cooldown period will be applied. This cooldown period gives the environment time to distribute work between available worker nodes.
...
As parent, the base image uses the \textit{nvidia/cuda:11.0-devel-ubuntu16.04} Docker image. The parent image runs Ubuntu\footnote{Enterprise Open Source and Linux - \url{https://www.ubuntu.com/} (Accessed: 2021-01-03)} in version 16.04. Additionally the CUDA Toolkit and the NVIDIA GPU driver are already installed.
...
% Apache Spark
Apache Spark will be installed at \textit{/opts/Spark}.
% Work dir
...
% RAPIDS
To enable GPU acceleration on all Apache Spark nodes, the base image will install the compiled Java files for the RAPIDS plugin at \textit{/opt/sparkRapidsPlugin} (for RAPIDS installation requirements, see \SubSec{subsec:04_rapids_req}). The .jar files can be downloaded in the maven repository.
% Discovery script
To enable Apache Spark to discover available GPUs, a GPU discovery script is needed (see \SubSec{subsubsec:04_spark_standalone_res-alloc} for details about resource allocation). This discovery script will be placed at \textit{/opt/sparkRapidsPlugin} as well. The discovery script is introduced at \Lst{lst:appendix_spark_gpu-discovery}.
