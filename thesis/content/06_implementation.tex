...
\subsubsection{Configuration for scaling Docker Containers}
%
...
\end{itemize}
% Explain example config
\Lst{lst:06_auto-scaler_config_container_example} provides an example of the configuration file using the container related settings. 
%example
...
\subsubsection{Sharing the Applications Source Code between Docker Containers}
% /builds volume
...
%
As explained in \Sec{subsec:04_background_gitlab_pipeline}, a GitLab CI/CD pipeline is configured by a \texttt{.gitlab-ci.yml} file in the root directory of the source code repository.
%
...
%\paragraph{}
%Intro
%At the start, the textit{train-model} job creates a mount-point to share the applications source code with other Docker containers.
% Isolation
%As mentioned before, Docker container run in isolation and do not share the same filesystem with each other. Therefore, the /builds directory is mounted to the host filesystem.
% The runner
