...
% Example master
\begin{lstlisting}[frame=single, label=lst:04_spark_standalone_launch-master, caption=Usage of master launch script, captionpos=b]
$ ./sbin/start-master.sh
\end{lstlisting}

% Example worker
\begin{lstlisting}[frame=single, label=lst:04_spark_standalone_launch-worker, caption=Usage of worker launch script, captionpos=b]
...
% Cluster mode
In cluster mode (introduced in SUBSECTION XY) the driver of a Spark application (see subsection AB) will be launched from one of the worker processes inside the cluster.
% When does it finish
...
% Example
\begin{lstlisting}[frame=single, label=lst:04_spark_standalone_submit_example, caption=Execution of a Spark Python application using the spark-submit executable, captionpos=b]
