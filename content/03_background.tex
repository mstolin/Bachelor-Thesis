...
\section{Prometheus}
% What is Prometheus
Prometheus is an open-source monitoring and alerting system that stores all data as time-series in a time-series database (TSDB) \cite{Prom2020Docs}. It follows the pull-based approach to scrape metrics from hosts and services \cite{Bastos2019Prom}.
...
% BESCHREIBEN WAS METRICS SIND UND ALERTS !!!!!
...
As \Fig{fig:prom_pull-based} demonstrates, a pull-based monitoring system scrapes metrics from services which make metrics available for the monitoring system. In this case, the monitoring system needs a list of all hosts and services to monitor \cite{Bastos2019Prom}.
...
The Prometheus ecosystem provides multiple components. Components can be optional, depending on the monitoring needs of the environment \cite{Bastos2019Prom}. The main components of a Prometheus system consist of Prometheus server, Alertmanager, service discovery, exporters and visualization \cite{Prom2020Docs}.


\paragraph{Prometheus server:} The Prometheus server is the main component of a Prometheus system. It is responsible to collect metrics as time-series data from targets and stores the collected data in the built-in TSDB. Additionally, the Prometheus server triggers alerts to the Alertmanager if a configured condition becomes true \cite{Prom2020Docs, Bastos2019Prom}.


\paragraph{Alertmanager:}
If an alerting condition becomes true, the Prometheus server generates and alert and pushes it to the Alertmanager. The Alertmanager routes the alert to the specific integration.
...
As mentioned before, Prometheus follows a pull-based approach to scrap metrics from targets and needs to know about the target hosts and services. ..... Therefore a an automated service-discovery is needed ... % Besser formulieren
